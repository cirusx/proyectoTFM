	...
	app.controller('authController',['$scope', '$rootScope', '$http', '$location', '$cookies',
		...
		$scope.recoverypassword = function() {
			...
			$http.get('http://localhost:8080/proyectoTFM/rest/users/recovery/'+ email.replace('@', '%40')).then(function(user) {
				...
				$scope.incorrectUser= false;
				delete $scope.user;
				$scope.recoverypasswordform.$setUntouched();
		...
		$scope.register = function() {
			...
			var email = $scope.user.email;
			$http.get('http://localhost:8080/proyectoTFM/rest/users/check/'+ email.replace('@', '%40')).then(function(user) {
				$scope.emailUsed = true;
				$scope.userCreated = false;

			}, function(user) {
				if($scope.emailUsed == false) {
